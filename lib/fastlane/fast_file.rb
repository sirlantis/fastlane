module Fastlane
  class FastFile
    ...
    def initialize(path = nil)
      ...
      UI.crash!("Could not find Fastfile at path '#{path}'") unless File.exist?(path)
    ...
    def method_missing(method_sym, *arguments, &_block)
      ...
      if class_ref && class_ref.respond_to?(:run)
        ...
        UI.crash!("Action '#{method_sym}' of class '#{class_name}' was found, but has no `run` method.")
      end
    end
    ...
    def import(path = nil)
      UI.crash!("Please pass a path to the `import` action") unless path
    ...
    def import_from_git(url: nil, branch: 'HEAD', path: 'fastlane/Fastfile')
      UI.crash!("Please pass a path to the `import_from_git` action") if url.to_s.length == 0
